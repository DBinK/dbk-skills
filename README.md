...
把 AI 编程 Agent 的协作规范沉淀为可复用、可触发的规则集，装进任何支持 [Agent Skills](https://agentskills.io) 规范的客户端。

## 为什么

用 AI 编程 Agent 开发，能力强，但判断不可靠。它可能没问就 `git commit`，照单全收评审意见，跳过格式化直接提交，还记不住上次踩的坑。你要么全程盯着它，要么接受它制造的烂摊子。

失控集中在四个环节，根源是 LLM 的**指令遵循倾向**：外部输入（你的话、评审意见、上游变更）进来，它往往直接执行，不做判断：
...
所有技能遵守同一原则：**人是决策者，Agent 是执行者**。落到下面的规则，分别约束上面的每个环节：
